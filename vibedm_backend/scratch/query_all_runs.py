...
async def main():
    ...
    async with sessionmaker() as session:
        # Fetch runs for the automation
        runs_res = await session.execute(
            text("select id, status, error, trigger_event, step_trace, created_at, finished_at from public.automation_runs where automation_id = '5fb9575f-870e-49f3-ae1c-3ea9059e73fa' order by created_at desc limit 10")
        )
        runs = runs_res.mappings().all()
        print("=== LATEST 10 RUNS FOR DM AUTOMATION ===")
        for r in runs:
            # Parse trigger event
